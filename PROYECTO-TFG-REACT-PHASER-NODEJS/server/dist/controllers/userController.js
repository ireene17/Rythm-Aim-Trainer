...
const verificarToken = (req, res, next) => {
    ...
    try {
        const decoded = jsonwebtoken_1.default.verify(token, "supersecreto");
        req.usuario = decoded;
        next();
    }
    catch (err) {
...
router.get('/ranking', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const usuarios = yield User_1.UserModel.find({}, {
            userName: 1,
            fotoPerfil: 1,
...
// PUT /api/usuario/:id
router.put("/:id", verificarToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    const { fotoPerfil, panel, descripcion } = req.body;
    try {
        const actualizado = yield User_1.UserModel.findByIdAndUpdate(id, { fotoPerfil, panel, descripcion }, { new: true });
        if (!actualizado) {
...
// GET /api/usuario/:id
router.get("/:id", verificarToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    try {
        const usuario = yield User_1.UserModel.findById(id)
        ...
        res.json(Object.assign(Object.assign({}, usuario.toObject()), { estadisticas: estadisticasCompletas }));
    }
    catch (err) {
...
router.post("/:id/completado", verificarToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
...
router.post("/actualizar-estadisticas", verificarToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    console.log("Datos recibidos:", req.body);
    try {
        const { userId, mapaId, mapa, puntos, aciertos, intentos, centroClicks, tiempoReaccion, rachaActual, rachaMax, resultado, precision } = req.body;
        const usuario = yield User_1.UserModel.findById(userId);
        if (!usuario) {
            res.status(404).json({ error: "Usuario no encontrado" });
            return;
        }
        ...
        const sesionesHistorial = stats.historialSesiones.map((s) => ({
        ...
        const sesionesPorCategoria = {};
        for (const sesion of sesionesHistorial) {
            const mapaDoc = yield Map_1.MapModel.findOne({ nombre: sesion.mapa });
            if (!mapaDoc || !mapaDoc.categoria)
                continue;
